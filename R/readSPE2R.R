readSPE2R <- structure(function(#Import Princeton Intruments (TM) SPE-file into R
  ...
  file,
  ### \link{character} (\bold{required}): spe-file name (including path), e.g. \cr
  ...
  frame.range, 
  ### \code{\link{vector}} (optional): limit frame range, e.g. select first 
  ### 100 frames by \code{frame.range = c(1,100)}
...
return(object)
  ...
  ##value<<
  ## Depending on the chosen option the functions returns three different type 
  ## of objects:\cr
  ##
  ...
  ##
  ## An object of type \code{\linkS4class{RLum.Data.Spectrum}} is returned. 
  ## Row sums are used to integrate all counts over one channel. 
  ##
  ...
  ##references<<
  ## Princeton Instruments, 2014. Princeton Instruments SPE 3.0 File Format Specification,
  ## Version 1.A,
  ...
  ##
  ## Hall, C., 2012: readSPE.m. \url{http://www.mathworks.com/matlabcentral/fileexchange/35940-readspe/content/readSPE.m}

  ##note<<
  ## \bold{The function does not test whether the input data are spectra or pictures
  ## for spatial resolved analysis!}\cr
  ##
