CW2pPMi<- structure(function(#Transform a CW-OSL curve into a pPM-OSL curve via interpolation under parabolic modulation conditions
  ...
  values,
  ### \code{\linkS4class{RLum.Data.Curve}} or \code{\link{data.frame}} (\bold{required}):
  ### \code{\linkS4class{RLum.Data.Curve}} or \code{data.frame} with measured curve data of
  ### type stimulation time (t) (\code{values[,1]}) and measured counts (cts) 
  ### (\code{values[,2]})
  
  P
  ### \code{\link{vector}} (optional): stimulation period in seconds. 
  ### If no value is given, the optimal value is estimated automatically (see details).
  ...
  ## The input \code{data.frame} consists of two columns: time (t) and count values 
  ## (CW(t))\cr\cr
  ## 
  ## \bold{Nomenclature}\cr\cr
  ...
  ## which contain no \code{NA} values and use these values for a linear 
  ## fit using \code{\link{lm}}.\cr\cr
  ## (5) Extrapolate values for t' < \code{min(t)} based on the previously 
  ## obtained fit parameters.
  ## The extrapolation is limited to two values. 
  ## Other values at the beginning of the transformed curve are set to 0.\cr\cr
  ...
  ## automatic estimation routine for \code{P}, i.e. provide no value for \code{P}.}
  ...
  ## with two additional info elements:
  ## \tabular{rl}{
  ...
  ##note<<
  ## According to Bos & Wallinga (2012), the number of extrapolated 
  ## points should be limited to avoid artificial intensity data. 
  ## If \code{P} is provided manually, not more than two points are extrapolated. 
