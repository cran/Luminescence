calc_CosmicDoseRate<- structure(function( # Calculate the cosmic dose rate
  ...
  depth,
  ### \code{\link{numeric}} (\bold{required}): depth of overburden (m). 
  ### For more than one absorber use \cr
  ### \code{c(depth_1, depth_2, ..., depth_n)}
  density,
  ### \code{\link{numeric}} (\bold{required}): average overburden 
  ...
  ### \code{\link{logical}} (with default): correct for 
  ### geomagnetic field changes after Prescott & Hutton (1994). Apply only when
  ### justified by the data.
  est.age = NA,
  ### \code{\link{numeric}} (with default): estimated age range (ka)
  ### for geomagnetic field change correction (0-80 ka allowed)
  half.depth = FALSE,
  ### \code{\link{logical}} (with default): How to overcome with 
  ...
  error = 10
  ### \code{\link{numeric}} (with default): general error (percentage) 
  ...
  if(length(hgcm)==1) {  
    ...
    ##note<<
    ## Despite its universal use the equation to calculate the cosmic dose rate
    ...
    ## of Physics G: Nuclear and Particle Physics 9, 1577-1595. \cr\cr
    ...
    ## age estimates. Ancient TL 27, 45-46. \cr\cr
    ## Prescott, J.R., Hutton, J.T., 1988. Cosmic ray and gamma ray dosimetry
    ## for TL and ESR. Nuclear Tracks and Radiation Measurements 14, \cr\cr
    ## 223-227.
    ...
    ## variations. Radiation Measurements 23, 497-500. \cr\cr
    ...
    ## altitude and depth dependences. PACT 6, 17-25.
